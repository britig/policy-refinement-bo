...
The landing pad is always at coordinates (0,0). The coordinates are the first two numbers in the self.state vector.
...
class LunarLander(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        shaping = \
            ...
            - 100*abs(self.state[4]) + 10*self.state[6] + 10*self.state[7]  # And ten points for legs contact, the idea is if you
...
def heuristic(env, s):
    ...
    Args:
        env: The environment
        s (list): The self.state. Attributes:
...
def demo_heuristic_lander(env, seed=None, render=False):
    with open('failure_trajectory_lunar.data', 'rb') as filehandle1:
            # read env_state
            failure_observations = pickle.load(filehandle1)
    set_seed = env.seed(failure_observations[0][2][0])
    print(f'set_seed ============={set_seed}')
    obs = failure_observations[0][0][0]
    total_reward = 0
    steps = 0
    s = env.reset()
    print(f'Position s before ============= {s}')
    '''print(f'Position x ============= {env.lander.position[0]}')
    print(f'Position y ============= {env.lander.position[1]}')
    print(f'Linear vel x ============= {env.lander.linearVelocity[0]}')
    print(f'Linear vel y ============= {env.lander.linearVelocity[1]}')
    env.lander.position[0] = env.lander.position[0]+10
    env.lander.position[1] = env.lander.position[1]+10
    env.lander.linearVelocity[0] = env.lander.linearVelocity[0]+3'''
    #s = env.step(0)[0]
    env.state = obs
    s = env.state
    print(f'Position s after ============= {s}')
    '''print(f'Position x changed ============= {env.lander.position[0]}')
    print(f'Position y changed ============= {env.lander.position[1]}')
    print(f'Position s after ============= {s}')'''
    while True:
        a = heuristic(env, s)
