...
import sys
from eval_policy import eval_policy, display
	...
	def rollout(self,subpolicy,failure_observations):
		...
		while t < self.timesteps_per_batch:
			...
			if subpolicy:
				...
				#reinitialize the environment after all the trajectories are exhausted for Bipedal-Walker
				if env_name == 'BipedalWalker-v3': 
					if(index == 0):
						self.index_count = 0
						self.env = set_environment(env_name,0)
					obs = failure_observations[index][0][0]
					index_fail = failure_observations[index][2]
					while self.index_count != index_fail:
						self.env.reset()
				...
				if env_name == 'LunarLanderContinuous-v2':
					...
					self.env.reset()
					disturbances = failure_observations[index][3]
					self.env.env.lander.position[0] = self.env.lander.position[0]+disturbances[0]
					self.env.env.lander.position[1] = self.env.lander.position[1]+disturbances[1]
					self.env.env.lander.linearVelocity[0] = self.env.lander.linearVelocity[0]+3
					obs = failure_observations[index][0][0]
				else:
					self.env.reset()
					obs = failure_observations[index][0][0]
			...
			for ep_t in range(self.max_timesteps_per_episode):
				...
				if subpolicy and self.env.unwrapped.spec.id == 'CartPole-v0':
					if obs[0]>1.5 or obs[0]<-1.5 :
						rew = rew - 10
				#Negative reward for the angle of Bipedal-Walker
				if subpolicy and self.env.unwrapped.spec.id == 'BipedalWalker-v3':
					if self.env.env.hull.angle > 2 or self.env.env.hull.angle < -0.8:
						rew = rew - 1

				#Add the negative penalty for the last state of the Lunar Lander
				if done:
					#Negative reward for going beyond the flag positions (-0.4 and +0.4) for 
					if subpolicy and self.env.unwrapped.spec.id == 'LunarLanderContinuous-v2':
						if obs[0]>0.4:
							rew = rew - (100*(0.4-obs[0]))
